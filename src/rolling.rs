...
#[inline]
fn update_crc(state: u64, input: u8) -> u64 {
    let i = (state ^ input as u64) & 0xff;
    // SAFETY: the table always has 256 entries, and 0 <= i < 256
    unsafe { CRC_TABLE.get_unchecked(i as usize) ^ (state >> 8) }
}

fn make_roll_table(window: usize) -> Vec<u64> {
    let mut table = Vec::with_capacity(256);
    for i in 0..=255 {
        let mut crc = 0;
        crc = update_crc(crc, i);
        for _ in 0..window {
            crc = update_crc(crc, 0);
        }
        table.push(crc)
    }
    table
...
impl RollingHash {
    pub fn new(window: usize) -> Self {
        RollingHash {
            ...
            roll_table: make_roll_table(window),
        }
    }
    ...
    pub fn update(&mut self, input: u8) -> u64 {
        let mut state = self.state;
        state = update_crc(state, input);
        if self.valid() {
            let outgoing = self.buf.pop_front().unwrap();
            state ^= self.roll_table[outgoing as usize];
        }
    ...
    pub fn feed(&mut self, input: &[u8]) -> u64 {
        for c in input.iter() {
            self.update(*c);
        }
        self.state
...
mod tests {
    ...
    #[test]
    fn should_match() {
        let s1 = "This is some example text to demonstrate the rolling hash.".as_bytes();
        let s2 = "Here is more text to demonstrate the rolling hash.".as_bytes();

        let mut hasher = RollingHash::new(16);
        let h1 = hasher.feed(&s1[..]);
        hasher.reset();
        let h2 = hasher.feed(&s2[..]);

        // last 16 bytes are "the rolling hash." and should have the same hash
        assert_eq!(h1, h2);
    }
}
